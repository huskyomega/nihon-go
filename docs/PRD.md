...
#### 單字單元
單元清單依等級各自獨立維護（`flashcard.js` / `vocab-study.js` 內的 `UNIT_ORDERS[LEVEL]`），
不同等級的分類不會互相套用：
...
**N4（共 19 個，對應 sigure.tw N4進階單字第 01～19 課主題）**：
數字・時間、人物・職稱、身體・身心、場所・方位、飲食、自然、交通工具、
服裝・飾品、電器、專業領域、休閒、物品、名詞綜合、形容詞、形容動詞、
副詞、動詞（他動詞）、動詞（自動詞）、動詞（自他對立）

新增等級時須同步在 `UNIT_ORDERS` 加入對應陣列，否則該等級的單元篩選列會是空的
（仍可用「全部」瀏覽，只是不能依單元篩選）。
...
#### 文法單元
單元清單依等級各自獨立維護（`grammar.js` / `grammar-study.js` 內的 `UNIT_ORDERS[LEVEL]`）：

**N5（共 12 個）**：
基本文型、い形容詞、な形容詞、動詞活用、助詞、て形的用法、
時間・順序、意向・勧誘・願望、疑問・限定、副詞・接続詞、普通體・語氣、比較・能力

**N4（共 16 個，對應 sigure.tw N4文法第 01～53 課，跳過第27課純複習內容）**：
名詞接續表現、形容詞・形容動詞、條件・假定、使役・受身、て形補助動詞、
動詞活用、様態・推量・伝聞、授受表現、敬語、意向・打算、原因・理由・目的、
複合動詞・衍生表現、命令・許可・禁止、形式名詞表現、副詞・接続詞、助詞
